...
class Grid:
    #M=Rows, N=Columns, P=Width of inner grid lines, W=, B=Border Width
    def __init__(self, obs_prob, M=15, N=15, P = 2, W = 21, B = 13):

        self.B = B
        self.color = [0.2588, 0.4039, 0.6980] #the RGB color in standard 0-255 format is approximately (66, 103, 178), which is a shade of blue.
        self.brightness = 1.8
        self.grid = []
        self.P = P
        self.W = W
        ...
        for i in range(self.M):
            ...
            for j in range(self.N):
                ...
                for c in range(3):
                    ...
                    grid_ij[:,:,c] = 1-(self.color[c]*self.obs_prob[i][j])



                #make inner cross lines
                grid_ij[:self.P//2,:,:] = 0 #top horizontal black lines
                grid_ij[-self.P//2:,:,:] = 0 #bottom horizontal black lines

                grid_ij[:,:self.P//2,:] = 0 #left vertical black lines
                grid_ij[:,-self.P//2:,:] = 0 #right vertical black lines
    ...
    #Retrieves the cell grid at position (x, y) with adjusted row indexing.
    def get_grid(self, x, y):
        grid = self.grid[self.M - y - 1][x]
        return grid


    #Updates a portion of the grid between start point sp and end point dp with a specified color.
    ...
    #Draws a line segment between two points sp and dp on the grid by determining the appropriate grid cells to update.
    ...
    #Draws a path through the grid by calling draw_one_step for each segment in the given sequence of coordinates.
    def draw_path(self, sequence, color = [0,0,1]):
        ...
        start_grid[self.P//2:-1*self.P//2,self.P//2:-1*self.P//2,0] += 1 # coloring the starting block red


        end_x, end_y = sequence[-1]
        end_grid = self.get_grid(end_x-1,end_y-1)
        end_grid[self.P//2:-1*self.P//2,self.P//2:-1*self.P//2] *= 0
        end_grid[self.P//2:-1*self.P//2,self.P//2:-1*self.P//2,1] += 1 # coloring the end block green

        for i in range(1,len(sequence)):
            self.draw_one_step(sequence[i-1],sequence[i], color)


    #Concatenates the grid cells into a single image, adds a border, and saves it as an image file.
